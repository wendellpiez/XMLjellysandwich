...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns="http://www.w3.org/2000/svg" exclude-result-prefixes="#all"
...
<!--    Pass in $set-time as a string. Errors if not hh:mm:ss (with zeroes).
    ...
    <xsl:template name="draw-clock">
        ...
        <svg id="clock" viewBox="0 0 400 300">
            ...
            <text x="200" y="250" text-anchor="middle" font-size="14" font-style="italic">Ist das nicht eine Uhr?</text>
            <text x="10" y="50" id="time-setting">
                <xsl:value-of select="format-time($now,'[h]:[m02] [Pn]')"/>
            </text>
            <g transform="translate(200 120)">
                <!--<circle r="90" fill="none" fill-opacity="0.2" stroke="steelblue" stroke-opacity="0.2"
                    stroke-width="10"/>-->
