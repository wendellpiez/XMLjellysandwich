...
var BrowserPlatform = (function () {
    ...
    var platform = {
         ...
         * If there is no pool entry for the uri, then do an asynchronous fetch, add an entry to the pool and then
         * do callback. If there is a pool entry for the uri, then do the callback straightaway.
         * @param uri of the document
         * @param callback is function callback(pool)
         * @param pool the documentPool
         */

        addXml: function (uri, callback, pool) {
            function poolFn(uri, doc) {
                //saxonPrint("Add to pool " + uri);
                pool[uri] = doc;
                callback(pool);
            }

            if (!pool[uri]) {
                platform.asyncGetXml(uri, poolFn);
            } else {
                //saxonPrint("Doc already in pool " + uri);
                callback(pool);
            }
        },

        /** Do callback on supplied document pool, possibly after first adding multiple entries.
         * For each URI in docUris, do addXml (which adds an entry to the pool if it is not already there) -
        ...
        addMultipleXml: function (docUris, pool, callback) {
            ...
            for (var i = 0; i < docUris.length; i++) {
                if (docUris[i]) {
                    platform.addXml(docUris[i], callbackOnLast, pool);
                } else {
                    counter--;
                }
            }
         ...
         * @param request is a JS Object representation of the HTTP request
         * @param callback is a function applied when the response has returned; the function takes one parameter -
         * the JS Object representation of the HTTP response
         */

        makeHttpRequest: function (request, callback) {
            ...
            function prepareRequest(request) {
                ...
                saxonPrint("HTTP request prepared");
                return request;
            }
            ...
            // HTTP state change handling: including handing response.
            http.onreadystatechange = function () {
                ...
                if (http.readyState === 4) {
                    if (http.status >= 200) {
                        saxonPrint("Saxon-JS processing httpRequest response");
                        ...
                        // ?? timeout if http.status is not >= 200
                        throw XError(http.statusText + ": " + request.href, "SXJS0006");
                    }
                } // ?? timeout if http.readyState is never 4
            };
...
var Atomic = (function () {
    ...
    function invalidDecimal(str, name) {
        throw new XError("Supplied value \"" + str + "\" is not a valid xs:" + name, "FOCA0002");
    }
...
var Expr = (function () {
    ...
    var expression = {
        ...
        doc: function (expr, context) {
            // TODO look at parent - treat expr with as document-node; or perhaps more reliably globalVariable with type document-node
            // see bug #3223
            // in this case make a new document, rather than a document fragment
        ...
        gVarRef: function (expr, context) {
            ...
            var gVar = co.actor;
            var name = gVar.getAttribute("name");
            ...
            if (co.value === null || co.value === undefined) {
        ...
        ifCall: function (expr, context) {
            ...
            if (name.local == "schedule-action") {
                ...
                // From 9.7.0.19 and 9.8.0.2 we check at compile time that only one of the attributes wait, document, http-request is used
                // Otherwise the precedence order is: http-request, document, wait
                if (httpRequest !== null) {
                    //throw XError("Use of @http-request on ixsl:schedule-action is not yet implemented in Saxon-JS", "SXJS0002", expr);

                    // Callback function called when the HTTP request returns a response
                    // response is the representation of the HTTP response as a JS Object
                    var responseFn = function (response) {
                        // Change the focus in c2 to be the map representation of the HTTP response
                        c2.focus = new Iter.Tracker(new Iter.Singleton(convertFromJS(response)));
                        c2.focus.next();
                        c2.currentMergeGroup = null;
                        c2.currentComponent = co;
                    ...
                    // httpRequest is a HashTrie, convert it to a JS Object before calling platform.makeHttpRequest()
                    SaxonJS.getPlatform().makeHttpRequest(convertToJS(httpRequest), responseFn);
                    ...
                    SaxonJS.getPlatform().addMultipleXml(documentAbsA, context.fixed.documentPool, fn);
        ...
        instance: function (expr, context) {
            ...
            while ((item = val.next()) !== null) {
                //saxonPrint(showValue(item) + " instance-of " + type.name);
        ...
        resultDoc: function (expr, context) {
            ...
            if (href === "") {
                ...
                if (!target) {
                    saxonPrint("Result document href=" + href + ": no such node");
                    return Iter.Empty;
                }
                if (replaceContent) {
                    target.innerHTML = "";
    ...
    function assembleParams(expr, oldContext, newContext) {
        ...
        withParams.forEach(function (wp) {
            ...
            //saxonPrint("New param " + pName + " = " + showValue(paramMap[tunnel][pName]));
            //if (tunnel == 1) {
            //    //saxonPrint("New tunnel param " + pName + " = " + showValue(paramMap[tunnel][pName]));
            //}
        });
                    ...
                    if (DU.isNSNode(child)) {
                        if (SaxonJS.getPlatform().inBrowser && parent instanceof HTMLElement) {
                            ...
                            if (child.prefix === "") {
                                ...
                                if (child.uri == "~" && KNOWN_URI[child.prefix] != null) {
    ...
    function convertFromJS(item, insideArr) {
        ...
        switch (typeof item) {
            case "undefined":
                return [];
            // previously return Atomic.untypedAtomic.fromString("");
            case "string":
    ...
    function convertToJS(item) {
        ...
        // map => JS object, with all the keys converted to strings
            ...
            //saxonPrint("item instanceof HashTrie " + showValue(item));
            var object = {};
        ...
        // JSValue(x) -> x
            ...
            //saxonPrint("item instanceof JSValue " + showValue(item));
            return item.value;
        } else {
    ...
    function serialize(seq, params) {
        ...
        if (!params) {
        } else if (params instanceof Iter.AnyIterator) {
            temp = SaxonJS.U.convertToJS(params.next());
        } else {
            temp = params;
        }
        for (var k in temp) {
        ...
        for (i = 0; i < regex.length; i++) {
            ...
            switch (c) {
                case "[":
                    ...
                    //         // rewrite [A-[B]] as (?![B])[A]
                    //         var bra = bracketDepth[bracketDepth.length - 1];
                    //         var lhs = output.substring(bracketDepth[bracketDepth.length-1]);
                    //         var ket = regex.indexOf("]", i);
                    //         var rhs = regex.substring(i+1, ket);
                    //         rhs = prepareRegex(rhs, flags);
                    //         output = output.substring(0, bra-1);
                    //         output += "(?![" + rhs + ")[" + lhs + "]";
                    //         i = ket;
                    //         saxonPrint("Rearranged regex: " + output);
                    //     } else {
                    //         error("Nested square brackets");
                    //     }
                    // }
...
var CoreFn = (function () {
    ...
    function applyFn(args) {
        ...
        // TODO Should we check that the key is an XDM array with only one item?
    ...
    return {
                ...
                if (item.matching) {
                    ...
                    for (g = 1; g < groups.length; g++) {
                        if (groups[g]) {
                            ...
                            group.setAttribute("nr", g); // TODO setAttributeNS ??
            ...
            if (!index) {
                ...
                for (var kd = 0; kd < keyDecls.length; kd++) {
                    ...
                    if (matchSpec.tagName == "p.nodeSet") {
                        ...
                        matchedNodes = matchedNodes.filter(function (item) {
                            cc2.focus = Iter.Solo(doc);
        ...
        trace: function (args) {
            var value = args[0].expand();
            saxonPrint((args[1] ? args[1].next().toString() + ": " : "") + showValue(value));
            return Iter.ForArray(value);
        },
        /*  transform() is currrently written to use an SEF as the stylesheet.
        ...
        transform: function (args, context, expr) {
            ...
            if (p != null) {
                options.stylesheetParams = Expr.convertToJS(p); // It will be back-converted!!!
            }
        ...
        "type-available": function (args, context, expr) {
            ...
            return Iter.oneBoolean(parts[0] == "Q{http://www.w3.org/2001/XMLSchema" && parts[1] in Atomic);
        },
    ...
    var interactiveFunction = function (local, args, context, expr) {
        ...
        var f = {
            ...
            get: function (args) {
                ...
                var z = getProp(x, props);
                if (typeof z == "undefined") {
                    saxonPrint("Warning ixsl:get: object property '" + y + "' not found");
                    return Iter.Empty;
            ...
            "set-property": function (args, context, expr) {
                ...
                if (typeof z == "undefined" || z === "") {
                    saxonPrint("Warning ixsl:set-property: '" + name + "' not found for supplied object");
                    return Iter.Empty;
                }
                z[last] = value;
                return Iter.Empty;
...
var XPath = (function () {
    ...
    function initialize() {
        ...
        if (inBrowser) {
            ...
            saxonPrint("Saxon-JS " + getProcessorInfo().productVersion + " in browser");
        } else {
    ...
    function applyStylesheet(stylesheet, sourceDoc, options) {
        ...
        if (sourceDoc) {
            ...
            //saxonPrint("Adding " + sourceUri + " to doc pool (doc " + sourceDoc._saxonBaseUri + ")");
        ...
        if (destination && !/^(appendToBody|prependToBody|replaceBody|application|native)$/.test(destination)) {
            saxonPrint("Warning: supplied transformation option not recognised - destination: " + destination);
        }
        ...
        if (initialMode !== "" && initialMode.substring(0, 2) != ("Q{")) {
            initialMode = "Q{}" + initialMode;
        }
        //saxonPrint("Initial mode = " + initialMode);

        var defaultIT = "Q{" + Expr.KNOWN_URI.xsl + "}initial-template"; //"Q{http://www.w3.org/1999/XSL/Transform}initial-template";
        ...
        if (initialFunction) {
            ...
            context.localVars = params.map(
                function (arg) {
                    return Expr.convertFromJS(arg);
                });
            context.localParams = {};
        ...
        if (destination == "application") {
            options.principalResult = Expr.convertToJS(output.expand());
    ...
    function transform(options, callback) {
        ...
        saxonPrint("Transform options supplied: " + listProps(options));
