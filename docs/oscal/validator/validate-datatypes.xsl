...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="pb:uri-reference" mode="pb:validate-type" as="xs:boolean">
      <xsl:variable name="extra" as="xs:boolean" select="true()"/>
      <xsl:sequence select="(. castable as xs:anyURI) and $extra"/>
   </xsl:template>
   ...
   <xsl:template match="pb:token" mode="pb:validate-type" as="xs:boolean">
      <xsl:variable name="extra" as="xs:boolean">
         <xsl:sequence select="matches(.,'^[\i-[:&#x10000;-&#xeffff;]][\c-[:&#x10000;-&#xeffff;]]*$')"/>
      </xsl:variable>
      <xsl:sequence select="(. castable as xs:string) and $extra"/>
   </xsl:template>
</xsl:stylesheet>
